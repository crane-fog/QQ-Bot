...
| `dm_notify_exclude` | 不私聊提醒的 Gitea 用户名（学号）名单，例如助教；这些人只保留群内通知 | 否 |
...
- **排除名单：** `dm_notify_exclude` 中的用户名（如助教）永不私聊。
- **降级：** 查不到 QQ 映射、未启用数据库或私聊发送失败时，在 `webhook_response_group` 群内 @（已知 QQ）并列出未绑定学号的用户。
