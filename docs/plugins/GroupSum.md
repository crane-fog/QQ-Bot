...
在 `plugins.toml` 中的 `[GroupSum]` 段落配置：
...
4. 使用系统人设（`prompt.j2`）和提取的消息内容构造上下文。
5. 调用 `self.bot.ai.generate("group_sum", ...)` 生成 JSON 格式的回应（模型由 `ai.toml` 中的 `[profile.group_sum]` 决定）。
6. 构建合并转发消息（Forward），包含总体描述和每个主题的详细列表（参与者、详情等）并发送。

## 外部依赖

- 需要 `MessageRecorder` 插件正常工作以提供历史消息。
- 依赖 `src/AIService.py` 提供的 `self.bot.ai.generate`，需在 `ai.toml` 中配置 `[profile.group_sum]` 及其使用的 `[provider.*]` 服务商（含 `api_key`）。
- 本地模板：`plugins/GroupSum/prompt.j2`
