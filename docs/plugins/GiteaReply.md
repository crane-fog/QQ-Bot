...
- 把消息按 CQ 码拆分为文本/媒体段：图片和视频作为 Gitea 评论附件上传，正文按出现顺序回填 `/attachments/...` 链接（图片内嵌渲染）；@人转 `@QQ号`、表情转 `[表情]`，其余 CQ 码不转发（`[CQ:file]` 也不是消息段——QQ 群文件走独立的上传事件，不会出现在消息文本里）
- 图片优先复用 MessageRecorder 预下载的本地文件：按 `event.sql_id` 查消息记录，取改写后 CQ 码的 `path` 参数直接上传；查不到记录或未启用 MessageRecorder 时，回退按 `url` 下载（限腾讯 CDN 域名，重定向逐跳校验，单文件上限 100MB）
...
- 复用 MessageRecorder 本地文件时，CQ 码的 `path` 由 OneBot 实现落盘产生，指向**其所在机器/容器的磁盘**：需要 Bot 与 OneBot 同机（或共享挂载卷）才能读到。跨机/容器隔离时该文件不可读，且改写后的码已删除 `url`、无法回退下载，对应图片会以占位文本标注
- 未启用 MessageRecorder 时图片由 Bot 直接从 QQ CDN 下载后上传，无需 Gitea 之外的凭据；下载仅允许腾讯 CDN 域名（`qpic.cn` / `qq.com` / `qq.com.cn` 后缀），其余 URL 一律拒绝
