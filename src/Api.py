...
class Api:
    ...
    class GroupService:
        ...
        def get_group_msg_history(self, group_id: int, count: int = 50) -> dict:
            ...
            response = requests.post(self.api.bot_api_address + "get_group_msg_history", json=params)
            return response.json()
        ...
        def get_group_recent_emoji_posters(self, group_id: int, emoji_id: int) -> dict:
            ...
            if msg_history_dict is None or msg_history_dict["status"] != "ok":
                raise ConnectionError(f"获取群聊天记录失败，group_id:{group_id}, status:{msg_history_dict['status']}")
            ...
            async def _query_all():
                async def _query_one(msg_id):
                    return msg_id, await self.api.asyncService.aget_msg_emoji_poster(msg_id, emoji_id)
        ...
        def get_group_list(self) -> dict:
            response = requests.post(self.api.bot_api_address + "get_group_list", json={})
            return response.json()
